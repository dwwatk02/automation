...
$OUTPUT_FILE = "asoc_ag_issues_export.csv"
$PAGE_SIZE = 500
...
# === GET APPS ===
function Get-Apps($token, $assetGroup) {
    ...
    while ($true) {
        $params = @{
            Page      = $page
            PageSize  = 50
            '$filter' = "AssetGroupName eq '$assetGroup'"
            '$select' = "Id,Name,AssetGroupName"
        }
...
# === HELPER: CUSTOM FIELD ===
function Get-CustomField($issue, $name) {
    foreach ($cf in $issue.CustomFields) {
        if ($cf.Name -eq $name) {
            return $cf.Value
        }
    }
...
Write-Host "Fetching applications for Asset Group: $AssetGroupName"
$apps = Get-Apps $token $AssetGroupName
